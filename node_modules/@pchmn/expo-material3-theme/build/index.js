...
export function useMaterial3Theme(params) {
    ...
    const [theme, setTheme] = useState(sourceColor ? createMaterial3Theme(sourceColor) : getMaterial3Theme(fallbackSourceColor));
    const updateTheme = (sourceColor) => {
 ...
 * @returns
 */
export function getMaterial3Theme(fallbackSourceColor = '#6750A4') {
    if (!isSupported) {
        return createThemeFromSourceColor(fallbackSourceColor);
    }
    const systemSchemes = ExpoMaterial3ThemeModule.getSystemTheme();
    if (systemSchemes) {
        return createThemeFromSystemSchemes(systemSchemes);
    }
    return createThemeFromSourceColor(fallbackSourceColor);
}
export async function getMaterial3ThemeAsync(fallbackSourceColor = '#6750A4') {
    if (!isSupported) {
        return createThemeFromSourceColor(fallbackSourceColor);
    }
    const systemSchemes = (await ExpoMaterial3ThemeModule.getSystemThemeAsync());
    if (systemSchemes) {
        return createThemeFromSystemSchemes(systemSchemes);
    }
    return createThemeFromSourceColor(fallbackSourceColor);
 ...
 * @returns
 */
export function createMaterial3Theme(sourceColor) {
